...
public class PhoneSMS {
	...
	public static String messageModel(String mobile_code,int modelType){
		...
		if(modelType==0){
			sbf.append("【大连停车位】恭喜您,您在本平台注册的用户账号已经通过了审核,可以登陆app进行使用!");
		}
		if(modelType==1){
			sbf.append("【大连停车位】验证码："+mobile_code+".有效期为3分钟,当前操作为登录!");
		}
		if(modelType==2){
			sbf.append("【大连停车位】验证码："+mobile_code+"。有效期为3分钟，当前操作为绑定手机!");
		}
		if(modelType==3){
			sbf.append("【大连停车位】验证码："+mobile_code+"。有效期为3分钟，当前操为密码修改!");
		}
		if(modelType==4){
			sbf.append("【大连停车位】验证码："+mobile_code+"。有效期为3分钟，当前操作为提现申请!");
		}
		if(modelType==5){
			sbf.append("【大连停车位】验证码："+mobile_code+"。有效期为3分钟,当前操作为绑定微信!");
		}
		if(modelType==6){
			sbf.append("【大连停车位】验证码："+mobile_code+"。有效期为3分钟，当前操作为注册!");
		}
		return sbf.toString();
	}
